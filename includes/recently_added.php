...
function recentlyAdded($sharedServer = NULL) {
	if (isset($_GET['width'])) {
	...
	$fileContents = getPmsData('/library/recentlyAdded?X-Plex-Container-Start=0' .
		'&X-Plex-Container-Size=' .	$containerSize, $sharedServer) or
		die ("<div class='alert alert-warning'>Failed to access Plex Media Server. " .
			"Please check your settings.</div>");
	$recentRequest = simplexml_load_string($fileContents[0]);
	}
						...
						echo "<div class='poster'><div class='poster-face'>";
							...
							if ($recentXml['thumb']) {
								echo "<img src='includes/img.php?img=".urlencode($recentThumbUrl);
									if ($sharedServer) { echo "&" .urlencode($fileContents[1]); }
									"' class='poster-face'>";
							} else {
...
// Only run this directly if called from index.php with $_GET=['action']
if($_GET=['action']) {
	recentlyAdded();
}
